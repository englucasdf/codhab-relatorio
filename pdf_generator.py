...
import base64 as _base64, io as _io, os as _os, json as _json

def _get_logo_codhab():
    for path in ["logos_b64.json", _os.path.join(_os.path.dirname(__file__), "logos_b64.json")]:
        if _os.path.exists(path):
            with open(path) as f:
                logos = _json.load(f)
            return _io.BytesIO(_base64.b64decode(logos["CODHAB"]))
    raise FileNotFoundError("logos_b64.json não encontrado")
...
def _get_logo_gdf():
    for path in ["logos_b64.json", _os.path.join(_os.path.dirname(__file__), "logos_b64.json")]:
        if _os.path.exists(path):
            with open(path) as f:
                logos = _json.load(f)
            return _io.BytesIO(_base64.b64decode(logos["GDF"]))
    raise FileNotFoundError("logos_b64.json não encontrado")
